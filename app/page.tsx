...
function parseVehicleType(value?: string) {
...
export default async function Home({ searchParams }: HomeProps) {
  ...
  const settings = await convex.query(api.settings.preferences, { sessionToken });
  const selectedLocation =
    location !== undefined
      ? location
      : settings.defaultPickupArea === "Any area"
        ? undefined
        : settings.defaultPickupArea;
  const selectedVehicleType = parseVehicleType(vehicleType ?? settings.preferredVehicleType);
  const wantsGravel = gravelReady === undefined ? settings.gravelReadyOnly : gravelReady === "true";
  ...
  const vehicles = await convex.query(api.vehicles.listPublished, {
  ...
  return (
    <div className="min-h-dvh w-full max-w-full overflow-x-clip bg-white text-black">
      <TopNav />
      <MobileAppHeader title="Carlink" subtitle="Book a local car" />
      <main className="w-full max-w-full pb-[calc(5rem+env(safe-area-inset-bottom))] md:pb-0">
        <section className="w-full px-4 pb-4 md:hidden">
          <form action="/" method="get" className="overflow-hidden rounded-[1.5rem] bg-[#f3f3f3]">
            <label className="block border-b border-black/10 px-4 py-3.5">
              <span className="text-xs font-medium text-[#5e5e5e]">Pickup</span>
              <select
                name="location"
                className="mt-1 w-full appearance-none bg-transparent text-[17px] font-semibold text-black outline-none"
            ...
            <div className="grid grid-cols-2 border-b border-black/10">
              <label className="block border-r border-black/10 px-4 py-3.5">
                <span className="text-xs font-medium text-[#5e5e5e]">Start</span>
                <input
                  name="startDate"
                  className="mt-1 w-full bg-transparent text-[15px] font-semibold text-black outline-none"
                  type="date"
            ...
            <button type="submit" className="m-2 min-h-12 w-[calc(100%-1rem)] rounded-full bg-black px-5 text-base font-semibold text-white">
              Search cars
            </button>
          </form>
        </section>

        <section className="mx-auto hidden w-full max-w-7xl gap-5 px-4 pb-5 pt-4 sm:px-6 md:grid md:grid-cols-[0.95fr_1.05fr] md:gap-8 md:py-12 lg:px-8">
          <div className="flex flex-col justify-center gap-5 md:gap-8">
            ...
            <form action="/" method="get" className="rounded-2xl bg-white p-4 ring-1 ring-black/10">
            ...
            <div className="relative hidden overflow-hidden rounded-[1.75rem] bg-black text-white md:block md:rounded-2xl">
              <div className="relative aspect-[1.08/1] md:aspect-auto md:min-h-[620px]">
                ...
                <div className="absolute inset-x-0 bottom-0 p-3 sm:p-8">
                  <div className="max-w-md rounded-2xl bg-white p-5 text-black">
        ...
        <section className="w-full px-4 py-4 sm:px-6 md:mx-auto md:max-w-7xl md:py-10 lg:px-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h2 className="text-2xl font-bold leading-tight md:text-3xl">Cars near you</h2>
            ...
            <div className="flex flex-wrap gap-2 pb-1">
              {[
                { label: "All", href: "/?location=&vehicleType=&gravelReady=false" },
                ...
                <Link
                  key={filter.label}
                  href={filter.href}
                  className="min-h-10 shrink-0 rounded-full bg-[#f2f2f2] px-4 text-sm font-semibold transition hover:bg-[#e2e2e2]"
                >
                  <span className="flex min-h-10 items-center">{filter.label}</span>
                </Link>
              ))}
            </div>
          </div>
          <div className="mt-3 grid gap-0 md:mt-6 md:grid-cols-3 md:gap-5">

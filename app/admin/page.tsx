import { approveOwnerAction, approveVehicleAction } from "@/app/actions/admin";
      ...
      <main className="pb-[calc(7.5rem+env(safe-area-inset-bottom))] md:pb-0">
        <section className="mx-auto max-w-7xl px-4 py-5 sm:px-6 md:py-12 lg:px-8">
          ...
          <div className="mt-6 grid gap-3 md:mt-8 md:grid-cols-4 md:gap-4">
            {[
              { name: "Owner applications", count: overview.queues.ownerApplications },
              { name: "Booking requests", count: overview.queues.bookingRequests },
              { name: "Vehicle reviews", count: overview.queues.vehicleReviews },
              { name: "Payout holds", count: overview.queues.payoutHolds },
            ].map((queue) => (
              <article key={queue.name} className="rounded-[1.5rem] bg-[#efefef] p-4 md:rounded-2xl md:p-5">
                ...
                <div className="mt-5 inline-flex rounded-full bg-white px-3 py-2 text-xs font-medium">Review</div>
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="mx-auto grid max-w-7xl gap-5 px-4 pb-12 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8">
          <div className="overflow-hidden rounded-[1.75rem] bg-white shadow-[0_16px_45px_rgba(0,0,0,0.06)] ring-1 ring-black/5 md:rounded-2xl md:shadow-none md:ring-black/10">
                ...
                {overview.bookings.length > 0 ? (
                  overview.bookings.map((booking) => (
                    <div key={booking._id} className="py-4">
                      ...
                      <p className="mt-2 text-[#5e5e5e]">N${booking.pricingBreakdown.totalAmount}</p>
                    </div>
                  ))
                ) : (
